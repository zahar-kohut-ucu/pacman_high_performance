...
import visualization3 as visualization3
...
def main():
    myMaze = maze3.Maze(m, n, ghostsAmount, cherries, numberOfBroken = numOfBroken, setSeed = True, seed = 3)
    tileSize = 20
    width = myMaze.trueN
    height = myMaze.trueM
    ...
    # print(myMaze.maze)
    # time.sleep(180)
    visualization3.initBoard(myMaze, screen, width, height, tileSize=tileSize)
    visualization3.drawBorders(screen, width, height, tileSize=tileSize)

    myMaze.spawnPacmans(pacmans)
    myMaze.spawnGhosts(ghosts)

    visualization3.spawnCreatures(screen, ghosts, pacmans, tileSize=tileSize)
    pygame.display.flip()
    ...
    while GAME:
        ...
        visualization3.redrawBoard(screen, myMaze, pacmans, ghosts, oldGhosts + oldPacmans, tileSize=tileSize)
        for pacman in pacmans:
            move = myMaze.checkMove(pacman)
            if move == 1:
