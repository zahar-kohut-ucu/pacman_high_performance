...
import visualization4 as visualization4
...
def main():
    ...
    myMaze = maze4.Maze(m, n, ghostsAmount, cherries, numberOfBroken = numOfBroken, setSeed = True, seed = 2)
    tileSize = 20
    width = myMaze.trueN
    height = myMaze.trueM
    ...
    # print(myMaze.maze)
    # time.sleep(180)
    visualization4.initBoard(myMaze, screen, width, height, tileSize=tileSize)
    visualization4.drawBorders(screen, width, height, tileSize=tileSize)

    myMaze.spawnPacmans(pacmans)
    myMaze.spawnGhosts(ghosts)

    visualization4.spawnCreatures(screen, ghosts, pacmans, tileSize=tileSize)
    pygame.display.flip()
    ...
    while GAME:
        ...
        visualization4.redrawBoard(screen, myMaze, pacmans, ghosts, oldGhosts + oldPacmans, tileSize=tileSize)
        for pacman in pacmans:
            move = myMaze.checkMove(pacman)
            if move == 1:
