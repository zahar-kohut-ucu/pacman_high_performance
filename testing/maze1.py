# Maze implementation
# Imports
from creatures1 import Ghost, Pacman
import node1 as node1
import random


class Maze:
    def __init__(self, m: int = 35, n: int = 35, ghostsAmount = 0, numberOfDots: int = 1000000000000, numberOfBroken = 20, setSeed: bool = False, seed: int = 100):
        if setSeed:
            random.seed(seed)
        self.m = m
        self.n = n
        ...
        self._maze = [[node1.Node(i, j, 0, node1.nodeStates1.wallState) for j in range(n)] for i in range(m)]
        self._freePositions = []
        self._dotsPosition = []
    ...
    def getNeighbours(self, current: node1.Node):
    ...
    def generateMaze(self, current: node1.Node):
        # getting neighbours
        ...
        for neighbor in neighbours:
            if isinstance(neighbor.getState(), node1.nodeStates1.wallState):
    ...
    def mazeExtend(self):
        for i in range(self.m):
            self._maze[i].append(node1.Node(i, self.n, 0, node1.nodeStates1.freeState))
            self._maze[i] += [node1.Node(i, self.n + 1 + j, 0, self.getNode(i, -(j + 2)).getStateClass()) for j in range(self.n)]
