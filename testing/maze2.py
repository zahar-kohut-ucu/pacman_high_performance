# Maze implementation
# Imports
from creatures2 import Ghost, Pacman
...
class Maze:
    def __init__(self, m: int = 35, n: int = 35, ghostsAmount = 0, numberOfDots: int = 1000000000000, numberOfBroken = 52, setSeed: bool = False, seed: int = 100):
        ...
        self._maze = [[node2.Node(i, j, 0, node2.nodeStates2.wallState) for j in range(n)] for i in range(m)]
        self._freePositions = []
        self._dotsPosition = []
    ...
    def getNeighbours(self, current: node2.Node):
    ...
    def generateMaze(self, current: node2.Node):
        # getting neighbours
        ...
        for neighbor in neighbours:
            if isinstance(neighbor.getState(), node2.nodeStates2.wallState):
    ...
    def mazeExtend(self):
        for i in range(self.m):
            self._maze[i].append(node2.Node(i, self.n, 0, node2.nodeStates2.freeState))
            self._maze[i] += [node2.Node(i, self.n + 1 + j, 0, self.getNode(i, -(j + 2)).getStateClass()) for j in range(self.n)]
