...
# Dijkstra
def dijkstraFindShortestPathTo(myMaze: maze1.Maze, pacman, dotPos):
    visited = set()
...
#AStar
def aStarFindShortestPathTo(myMaze: maze1.Maze, pacman, dotPos):
    visited = set()
    ways = [(1, 0), (-1, 0), (0, 1), (0, -1)]
    pacmanPos = pacman.getCoordinates()
...
def getNextPacmanMove(myMaze: maze1.Maze, pacman: creatures1.Pacman, algo: int = 0):
    if not algo:
        algo = pacman.getTeam()
...
def getGhostNextMove(myMaze: maze1.Maze, ghost: creatures1.Ghost, pacmans):
    # look for Pacman
