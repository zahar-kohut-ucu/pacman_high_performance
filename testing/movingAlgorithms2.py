...
# Dijkstra
def dijkstraFindShortestPathTo(myMaze: maze2.Maze, pacman, dotPos):
    visited = set()
...
def aStarFindShortestPathTo(myMaze: maze2.Maze, pacman, dotPos):
    visited = set()
    ways = [(1, 0), (-1, 0), (0, 1), (0, -1)]
    pacmanPos = pacman.getCoordinates()
...
def getNextPacmanMove(mypool, myMaze: maze2.Maze, pacman: creatures2.Pacman, thrds: int = 6, algo: int = 0):
    if not algo:
        algo = pacman.getTeam()
    ...
    items = [(myMaze, pacman, dot) for dot in dots]
...
def getGhostNextMove(myMaze: maze2.Maze, ghost: creatures2.Ghost, pacmans):
    # look for Pacman
