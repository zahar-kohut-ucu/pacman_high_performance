...
def drawTile(screen: pygame.Surface, tile: node1.Node, tileSize: int = 20):
    team = tile.getTeam()
    x, y = tile.getCoordinates()
    if  team < 1:
        team = ''

    iconImage = pygame.image.load(f"./icons/{str(tile.getState())}{team}.png")
    screen.blit(iconImage, ((y + 1) * tileSize, (x + 1) * tileSize))
    

def initBoard(maze: maze1.Maze, screen: pygame.Surface, width: int, height: int, tileSize: int = 20):
    screen.fill((0, 0, 0))
...
def drawBorders(screen: pygame.Surface, width: int, height: int, tileSize: int = 20):
    for x in range(height+2):
        iconImage = pygame.image.load(f"./icons/wall.png")
...
def spawnCreatures(screen: pygame.Surface, ghosts: list, pacmans: list, tileSize: int = 20):
    for ghost in ghosts:
        ...
        iconImage = pygame.image.load(f"./icons/ghost{ghost.getTeam()}.png")
        screen.blit(iconImage, ((y + 1)*tileSize, (x + 1)*tileSize))
    
    for pacman in pacmans:
        x, y = pacman.getCoordinates()
        iconImage = pygame.image.load(f"./icons/pacman{pacman.getTeam()}.png")
        screen.blit(iconImage, ((y + 1)*tileSize, (x + 1)*tileSize))


def redrawBoard(screen: pygame.Surface, maze: maze1.Maze, pacmans, ghosts, coordsOld: list[int], tileSize=20):
