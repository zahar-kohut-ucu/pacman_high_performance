...
def drawTile(screen: pygame.Surface, tile, x, y, tileSize: int = 20):
    ...
    if team < 1:
        team = ''
    iconImage = pygame.image.load(f"./icons/{states[tile]}{team}.png")
    screen.blit(iconImage, ((y + 1) * tileSize, (x + 1) * tileSize))
    

def initBoard(maze: maze4.Maze, screen: pygame.Surface, width: int, height: int, tileSize: int = 20):
    screen.fill((0, 0, 0))
...
def spawnCreatures(screen: pygame.Surface, ghosts: list, pacmans: list, tileSize: int = 20):
    for ghost in ghosts:
        x, y = ghost[0], ghost[1]
        iconImage = pygame.image.load(f"./icons/ghost{ghost[2]}.png")
        screen.blit(iconImage, ((y + 1)*tileSize, (x + 1)*tileSize))
    
    for pacman in pacmans:
        x, y = pacman[0], pacman[1]
        iconImage = pygame.image.load(f"./icons/pacman{pacman[2]}.png")
        screen.blit(iconImage, ((y + 1)*tileSize, (x + 1)*tileSize))


def redrawBoard(screen: pygame.Surface, maze: maze4.Maze, pacmans, ghosts, coordsOld: list[int], tileSize=20):
